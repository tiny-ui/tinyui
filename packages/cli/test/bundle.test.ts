...
describe("tinyui bundle", () => {
    ...
    it("writes current.json next to an immutable <version>/ holding manifest.json and the bytecode", async () => {
        ...
        const result = await bundle({ dist, hostVersion: "1", signingKey, rollout: 30 });
        ...
        const manifest = JSON.parse(await readFile(result.manifest, "utf8")) as Manifest & { hostVersion: string };
        assert.equal(manifest.hostVersion, "1");
    ...
    it("signs the raw bytes of <version>/manifest.json, so the public key verifies the file as stored", async () => {
        ...
        const result = await bundle({ dist, hostVersion: "1", signingKey });
        const pointer = JSON.parse(await readFile(result.pointer, "utf8")) as Pointer;
    ...
    it("defaults rollout to 100 and lets it change without touching the signature", async () => {
        ...
        await assert.rejects(bundle({ dist, hostVersion: "1", signingKey, rollout: 101 }), /rollout must be an integer from 0 to 100/);
    });
    ...
    it("refuses a signing key that does not match the package's publicKey, or a publicKey that is not P-256", async () => {
        ...
        await assert.rejects(bundle({ dist, hostVersion: "1", signingKey: other }), /does not match the publicKey/);
        ...
        await assert.rejects(bundle({ dist: wrongCurve, hostVersion: "1", signingKey: p384Key }), /not a P-256/);
    });
    ...
    it("keeps a version immutable: the same build again is fine, a different one is refused", async () => {
        ...
        const first = await bundle({ dist, hostVersion: "1", signingKey, rollout: 10 });
        const before = await stat(first.manifest);
        const again = await bundle({ dist, hostVersion: "1", signingKey, rollout: 90 });
    ...
    it("never lets a version, host version or file path leave its directory", async () => {
        const escaping = await fakeDist(join(tmp, "dist-escape"), (m) => { m.version = "../../escape"; });
        await assert.rejects(bundle({ dist: escaping, hostVersion: "1", signingKey }), /manifest version must be a single path segment/);
        const dot = await fakeDist(join(tmp, "dist-dot"), (m) => { m.version = ".."; });
        await assert.rejects(bundle({ dist: dot, hostVersion: "1", signingKey }), /manifest version must be a single path segment/);
        const dist = await fakeDist(join(tmp, "dist-hostVersion-escape"));
    ...
    it("refuses a --js-only build, a manifest from an older CLI and a bad host version", async () => {
        ...
        await assert.rejects(bundle({ dist, hostVersion: "1/2", signingKey }), /host version must be a positive integer/);
        // an App version is the likeliest wrong value, and the one this rule exists for
        for (const appVersion of ["1.2.0", "1.2", "01", "0"]) await assert.rejects(bundle({ dist, hostVersion: appVersion, signingKey }), /it is not the App version/);
    });
